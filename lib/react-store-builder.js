(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		define("react-store-builder", ["react"], factory);
	else if(typeof exports === 'object')
		exports["react-store-builder"] = factory(require("react"));
	else
		root["react-store-builder"] = factory(root["react"]);
...
// CONCATENATED MODULE: ./src/testUtils.js
...
var testUtils_mockStore = function mockStore(useStoreCallback, onRenderCallback) {
  var store; // create a mock component,
  // is the only way to use a hook from tests

  var ComponentMock = function ComponentMock() {
    store = useStoreCallback();
    onRenderCallback(store);
    return external_react_default.a.createElement("div", null, "mock");
  };

  var node = document.createElement('div');
  react_dom_default.a.render(external_react_default.a.createElement(ComponentMock, null), node);
};
/**
...
var storeMockBuilder = function storeMockBuilder() {
  var _store = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

  var store = _store;

  var setMock = function setMock(moduleName, key, object) {
    store = moduleName ? _objectSpread({}, store, _defineProperty({}, moduleName, _objectSpread({}, store[moduleName], _defineProperty({}, key, object)))) : _objectSpread({}, store, _defineProperty({}, key, object));
    return storeMockBuilder(store);
  };

  return {
    mockState: function mockState(moduleName, state) {
      return setMock(moduleName, 'state', state);
    },
    mockGetters: function mockGetters(moduleName, getters) {
      return setMock(moduleName, 'getters', getters);
    },
    mockActions: function mockActions(moduleName, actions) {
      return setMock(moduleName, 'actions', actions);
    },
    mockDispatch: function mockDispatch(moduleName, dispatch) {
      return setMock(moduleName, 'dispatch', dispatch);
    },
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "StoreBuilder", function() { return src_StoreBuilder; });
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "mockStore", function() { return src_mockStore; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "storeMockBuilder", function() { return src_storeMockBuilder; });
...
var postBuild = function postBuild(store) {
  if (window) {
    window.$store = store;
  }
...
var src_StoreBuilder =
/*#__PURE__*/
function () {
  function StoreBuilder(state, reducer, namespace) {
  ...
  _createClass(StoreBuilder, [{
    ...
    key: "withGetters",
    value: function withGetters(getters) {
      this.getters = getters;
      return this;
    }
  }, {
    key: "withSubmodule",
    value: function withSubmodule(module) {
    ...
    value: function build() {
      ...
      return function (initialState, rootStore) {
        var _useReducer = Object(external_react_["useReducer"])(_this.reducer, initialState || _this.state),
            _useReducer2 = _slicedToArray(_useReducer, 2),
            state = _useReducer2[0],
            dispatch = _useReducer2[1];

        var getters = _this.getters(state);
        ...
        var dispatchAction = function dispatchAction(actionName, payload) {
          return store.actions[actionName](payload);
        };

        store.dispatchAction = dispatchAction;
        store.actions = _this.actions({
          dispatch: dispatch,
          state: state,
          getters: getters,
          rootStore: rootStore,
        ...
        var namespacedStore = _this.namespace ? src_defineProperty({}, _this.namespace, store) : store;

        var submodules = _this.submodules.map(function (mod) {
          return mod(null, store);
        }) // call hook
        .reduce(function (prev, curr) {
          return src_objectSpread({}, prev, {}, curr);
        }, {}); // transform to a map of modules


        return postBuild(src_objectSpread({}, namespacedStore, {}, submodules));
      };
    }
  }]);
...
var mapActions = function mapActions(store, namespace) {
  if (namespace) {
...
var mapState = function mapState(store, namespace) {
  if (namespace) {
...
var mapGetters = function mapGetters(store, namespace) {
  if (namespace) {
...
var getDispatch = function getDispatch(store, namespace) {
  if (namespace) {
...
var src_mockStore = testUtils_mockStore,
    src_storeMockBuilder = storeMockBuilder;
